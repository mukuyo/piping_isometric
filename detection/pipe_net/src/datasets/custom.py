...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(
        self, dataset_dir, dataset_dir2, class_names, train, img_size, bbox_format, augmentation=None,
    ):
        super().__init__()
        self.class_names = class_names
        self.img_size = img_size
        self.max_labels = 50
        self.init_dataset(dataset_dir, dataset_dir2, train, bbox_format)
        self.init_augmentation(augmentation, bbox_format="coco")

    def __getitem__(self, index):
        img_path, bboxes, class_ids = self.samples[index]
        img_path2, bboxes2, class_ids2 = self.samples2[index]
        ...
        img2, _ = utils.letterbox(
            img2, self.img_size, jitter=self.jitter, random_placing=self.random_placing
        )
        ...
        return img, img2, labels, pad_info

    def __len__(self):
        return len(self.samples)

    def init_dataset(self, dataset_dir, dataset_dir2, train, bbox_format):
        ...
        for img_name in img_names:
            ...
            img_path2 = dataset_dir2 / "images" / img_name
            ...
            self.samples2.append((img_path2, bboxes, class_ids))
